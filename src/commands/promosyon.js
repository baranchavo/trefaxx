...
module.exports = {
  ...
  dbl: false, 
  name: ['promosyon'],  
  description: 'TreFax Uptime promosyon sistemi.',
  options: [
    { 
      name: "oluştur",
...
async execute(client, interaction) {
  ...
  const sahipp = new EmbedBuilder()
    .setColor(Colors.Red)
    .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
    .setDescription(`${Emojis.Çarpı} Bu komutu sadece geliştiriciler kullanabilir.`)
    .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
    .setTimestamp()
  if(interaction.user.id !== "525816524729221162" && interaction.user.id !== "525816524729221162" && interaction.user.id !== "525816524729221162" && interaction.user.id !== "525816524729221162") return await interaction.followUp({embeds: [sahipp]})
  ...
  const KodListe = new EmbedBuilder()
    ...
    .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
    .setDescription(`> TreFax Uptime promosyon kodları listesi`)
    .addFields(
      {
