const { Discord, EmbedBuilder, ChannelType, ButtonBuilder, ActionRowBuilder, ButtonStyle, ModalBuilder, TextInputStyle, TextInputBuilder, InteractionType, PermissionsBitField, StringSelectMenuBuilder, SlasFRommandBuilder, ContextMenuCommandBuilder, ApplicationCommandType } = require ("discord.js")
...
module.exports = {
  ...
  dbl: false, 
  name: ['market'],  
  description: 'TreFax Uptime marketini görüntülersiniz.',
...
async execute(client, interaction) { 
  ...
  const Butonlar = new ActionRowBuilder()
    ...
    new ButtonBuilder()        
      .setURL(`https://discord.gg/ufvzVU4GjW`)
    ...
    .addFields(
      {
        ...
        value: `+1 link ekleme hakkı - **30 FR**`
      },
      {
        name: `${Emojis.Elmas} Premium üyelik`,
        value: `1 aylık premium üyeliği - **300 FR**`
      },
      {
        name: `${Emojis.Para} FR`,
        value: `FR almak için destek sunucusunda kurucu veya geliştiriciler ile iletişime geçiniz.`
      })
    ...
    if(interaction.customId === `linklimital_${interaction.user.id}`) {
        ...
        await interaction.awaitModalSubmit({ filter: (interaction) => interaction.customId === `limitalmaform`, time: 60000 }).then(async (interaction) => {
          ...
          const ParaYok = new EmbedBuilder()
            .setColor(Colors.Red)
            .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
            .setDescription(`${Emojis.Çarpı} Almak istediğin **${Limit}** link limit hakkı için **${Limit*30} FR** gerekli.`)
            .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
            .setTimestamp()
          ...
          const alındıı = new EmbedBuilder()
            .setColor(Colors.Green)
            .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
            .setDescription(`${Emojis.Tik} **${Limit}** link limit hakkı **${Limit*30} FR** karşılığında satın alındı.`)
            .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
            .setTimestamp()
          await interaction.followUp({embeds: [alındıı]})
      ...
      if(interaction.customId === `premiumal_${interaction.user.id}`) {
        ...
        if(Premium) {
          ...
          const ParaYok = new EmbedBuilder()
            .setColor(Colors.Red)
            .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
            .setDescription(`${Emojis.Çarpı} Premium almak için **300 FR** gerekli.`)
            .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
            .setTimestamp()
