  ...
  async execute(client, interaction) {
    ...
    if (interaction.options.getSubcommand() === 'ekle') {
      ...
      await interaction.awaitModalSubmit({ filter: (interaction) => interaction.customId === `linkeklemeform`, time: 60 * 60 * 1000 }).then(async (interaction) => {
        ...
        const Schema = require("../schhema/link")
        new Schema({
          link: link,
    ...
    if (interaction.options.getSubcommand() === 'admin-liste') {
      ...
      const Sahip = new EmbedBuilder()
        ...
        .setFooter({ text: client.user.username, iconURL: client.user.avatarURL() })
        .setTimestamp()
      if (interaction.user.id !== "525816524729221162" && interaction.user.id !== "525816524729221162" && interaction.user.id !== "525816524729221162" && interaction.user.id !== "525816524729221162") return await interaction.reply({ embeds: [Sahip] })
